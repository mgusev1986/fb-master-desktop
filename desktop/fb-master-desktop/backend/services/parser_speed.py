...
# turbo=0.08 — как в 2.64, где парсер «летал» (~170-380мс/раунд). FB при этом
# успевает рендерить виртуальный список благодаря **стартовой паузе 3с** в
# scroll_friends_page (восстановлена в 2.79), а не из-за длинных пауз между
# раундами. fast=0.5 (≈1.5-2.5с/раунд) — стабильно при обычной нагрузке.
