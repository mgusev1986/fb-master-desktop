...
async def fb_account_import_automation(
    ...
    if cookie_parsed:
        acc = FBAccount(
            ...
            profile_dir=str(_profiles_base(db) / "__new__"),
            fb_login_username=(cookie_parsed.fb_user_id or cookie_parsed.login).strip(),
    ...
    if cookie_parsed:
        return _redirect_flash(
            "Аккаунт по cookies импортирован: сессия записана в базу, User-Agent сохранён в карточке "
            "(из cookie-строки или по умолчанию для чистого JSON). "
            "Свободный слот 1–3 назначен автоматически, если был; иначе аккаунт в очереди. "
            "Рекомендуется при необходимости нажать «Проверить сессию».",
            "ok",
        )
