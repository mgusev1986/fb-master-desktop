"""LinkedIn template AI — обёртка над общим `complete_llm` FB Master.

4 операции (все async):
  * generate_variations(text, n=3)
  * rewrite(text, intent)
  * shorten(text, max_chars)
  * soften(text)

Сохраняет LinkedIn-плейсхолдеры буква в букву.
Берёт ключи и модель через тот же `effective_*_api_key` / `cabinet_settings`,
что и FB-template-variations — никакой собственной конфигурации.
"""
...
_SYSTEM_PRESERVE_PLACEHOLDERS = (
    "Ты помощник для шаблонов outreach-сообщений в LinkedIn. "
    "В тексте могут быть плейсхолдеры в фигурных скобках: "
    f"{_PLACEHOLDER_HINT}. Сохраняй их буква в букву, не заменяй и не убирай. "
...
def _resolve_llm_keys_and_model(db: Session) -> tuple[str, str, str, str]:
    """Возвращает (provider, openai_key, google_key, model).

    Использует те же `load_ui_settings` (provider/model) и
    `effective_*_api_key` (ключи), что Messenger AI и FB-template-variations.
    """
...
async def _call_llm_with_fallback(
    db: Session, *, system: str, user: str, response_mime_json: bool = False
) -> str:
    ...
    for p in order:
        ...
        try:
            ...
            last_err = str(e)
            continue
    raise LLMError(last_err or "Нет доступного LLM-ключа (задайте OPENAI_API_KEY или GOOGLE_API_KEY)")
...
async def generate_variations(db: Session, body: str, *, n: int = 3) -> dict[str, Any]:
    ...
    n = max(1, min(8, int(n)))
    user_msg = (
        f"Сгенерируй {n} РАЗНЫХ формулировок для следующего outreach-сообщения. "
        "Сохрани смысл, тон и язык, плейсхолдеры обязательны. "
        "Ответь ТОЛЬКО JSON-массивом из строк, без пояснений и без markdown-блока.\n\n"
        f"Исходный текст:\n{body}"
    )
    try:
        raw = await _call_llm_with_fallback(
            db,
            system=_SYSTEM_PRESERVE_PLACEHOLDERS,
            user=user_msg,
...
async def _single_op(db: Session, body: str, instruction: str) -> dict[str, Any]:
    ...
    try:
        raw = await _call_llm_with_fallback(
            db, system=_SYSTEM_PRESERVE_PLACEHOLDERS, user=user_msg
        )
    except LLMError as e:
        return {"ok": False, "reason_code": "llm_error", "details": str(e)[:200]}
...
async def rewrite(db: Session, body: str, intent: str) -> dict[str, Any]:
    ...
    return await _single_op(db, body, f"Перепиши следующий outreach-текст в стиле «{intent}», тот же смысл.")


async def shorten(db: Session, body: str, *, max_chars: int = 400) -> dict[str, Any]:
    return await _single_op(db, body, f"Сократи следующий outreach-текст до примерно {int(max_chars)} символов, без потери смысла.")


async def soften(db: Session, body: str) -> dict[str, Any]:
    return await _single_op(
        db, body,
        "Перепиши следующий outreach-текст: убери маркетинговый/спам-тон, "
        "избегай агрессивных слов («гарантирую», «срочно», «безусловно лучше»), "
        "сделай тон уважительным и по делу.",
    )
