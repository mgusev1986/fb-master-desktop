"""Twitter outreach builder — кампании + queue + render placeholders."""
...
def list_campaigns(db: Session, organization_id: int) -> list[TwitterOutreachCampaign]:
    return (
        db.query(TwitterOutreachCampaign)
        .filter(TwitterOutreachCampaign.organization_id == organization_id)
        .order_by(TwitterOutreachCampaign.created_at.desc())
        .all()
    )


def get_campaign(db: Session, campaign_id: int) -> TwitterOutreachCampaign | None:
    return db.get(TwitterOutreachCampaign, int(campaign_id))


def create_campaign(db: Session, organization_id: int, **fields: Any) -> TwitterOutreachCampaign:
    camp = TwitterOutreachCampaign(
...
def _render(template_body: str, lead: TwitterLead) -> str:
    ctx = {
...
def build_queue(db: Session, campaign_id: int, lead_ids: list[int], target_tweet_url: str | None = None) -> dict[str, Any]:
    ...
    tpl = db.get(TwitterTemplate, camp.template_id)
    if tpl is None:
        return {"created": 0, "skipped": 0, "errors": ["template_missing"]}

    existing = {
        row[0]
        for row in db.query(TwitterOutreachQueueItem.lead_id)
        .filter(TwitterOutreachQueueItem.campaign_id == camp.id)
        .all()
    }
    created = 0
    skipped = 0
    for lid in lead_ids:
        ...
        if lid_int in existing:
            skipped += 1
            continue
        lead = db.get(TwitterLead, lid_int)
...
def list_queue_items(db: Session, campaign_id: int) -> list[TwitterOutreachQueueItem]:
    return (
        db.query(TwitterOutreachQueueItem)
        .filter(TwitterOutreachQueueItem.campaign_id == campaign_id)
...
def set_queue_item_state(db: Session, item_id: int, state: str, error: str | None = None) -> TwitterOutreachQueueItem | None:
    item = db.get(TwitterOutreachQueueItem, int(item_id))
    if item is None:
        return None
