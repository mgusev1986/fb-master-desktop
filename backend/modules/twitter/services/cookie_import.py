"""Парсер импортируемых Twitter/X-аккаунтов.

Поддерживаемые форматы (одна строка = один аккаунт):

1) JSON-объект:
   {"label":"@handle","handle":"handle","cookies":[...],
    "proxy_url":"http://user:pass@host:port","user_agent":"...","full_name":"..."}
...
Обязательные cookies: `auth_token` И `ct0` (CSRF). Без них API X-веба
блокирует mutations (DM/reply) на CSRF-проверке.
"""
...
def _normalize_cookies(raw: Any) -> list[dict[str, Any]]:
    ...
    for c in raw:
        ...
        if not name or value is None:
            continue
        # X использует .x.com (новый) и .twitter.com (legacy). Нормализуем
        # на .x.com если домен не задан явно.
        domain = (c.get("domain") or ".x.com").strip() or ".x.com"
...
def parse_text(blob: str) -> list[ParsedTwitterAccount]:
    items: list[ParsedTwitterAccount] = []
    if not blob:
        return items
    for raw_line in blob.splitlines():
        ...
        try:
            ...
            items.append(ParsedTwitterAccount(label=line[:60], error=str(e)))
    return items


def _parse_line(line: str) -> ParsedTwitterAccount:
    if "\t" in line and not line.startswith("{") and not line.startswith("["):
        ...
        return ParsedTwitterAccount(
            label=label or "twitter",
            cookies=cookies,
    ...
    if line.startswith("["):
        ...
        if miss:
            raise ValueError(f"Не хватает cookies: {', '.join(miss)}")
        return ParsedTwitterAccount(label="twitter-account", cookies=cookies)
    ...
    if line.startswith("{"):
        ...
        label_default = f"@{handle}" if handle else "twitter-account"
        return ParsedTwitterAccount(
...
__all__ = ["ParsedTwitterAccount", "parse_text"]
