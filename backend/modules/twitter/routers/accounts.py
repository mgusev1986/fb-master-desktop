...
router = APIRouter(prefix="/twitter/accounts")
api_router = APIRouter(prefix="/twitter/api/accounts")
...
async def accounts_index(request: Request):
    ...
    return _tpl(request).TemplateResponse(
        "twitter/accounts/list.html",
        {
            "request": request,
            "user": request.session.get("user"),
            "page_id": "twitter_accounts_list",
            "accounts": accounts,
            "summary": summary,
...
async def accounts_probe(account_id: int, request: Request):
    ...
    try:
        ...
        db.close()
    return RedirectResponse(f"/twitter/accounts?notice=probe_{code}", status_code=303)
...
async def accounts_delete(account_id: int, request: Request):
    ...
    try:
        ...
        db.close()
    return RedirectResponse("/twitter/accounts?notice=deleted", status_code=303)
