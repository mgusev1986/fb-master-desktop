"""Twitter templates router (CRUD + AI)."""
...
router = APIRouter(prefix="/twitter/templates")
...
async def templates_index(request: Request):
    ...
    return _tpl(request).TemplateResponse(
        "twitter/templates/list.html",
        {
            "request": request,
            "user": request.session.get("user"),
            "page_id": "twitter_templates",
            "templates": items,
...
async def templates_create(
    ...
    try:
        try:
            ...
            return RedirectResponse("/twitter/templates?notice=missing_body", status_code=303)
    finally:
        db.close()
    return RedirectResponse("/twitter/templates?notice=created", status_code=303)
...
async def templates_delete(template_id: int, request: Request):
    ...
    try:
        ...
        db.close()
    return RedirectResponse("/twitter/templates?notice=deleted", status_code=303)
...
async def templates_ai(
    ...
    """AI ops: variations / rewrite / shorten / soften."""
    from backend.modules.twitter.services import template_ai
    ...
    try:
        ...
        if op == "variations":
            ...
            if op == "rewrite":
                ...
                res = await template_ai.shorten(db, tpl.body or "", max_chars=280)
            else:
