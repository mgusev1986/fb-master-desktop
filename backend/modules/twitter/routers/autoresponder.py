"""Twitter AI autoresponder UI + endpoints."""
...
from backend.modules.twitter.services import autoresponder as ar_svc
from backend.modules.twitter.services import leads as leads_svc
from backend.modules.twitter.services import personas as personas_svc

router = APIRouter(prefix="/twitter/autoresponder")
...
async def autoresponder_index(request: Request):
    ...
    try:
        ...
        personas = personas_svc.list_personas(db, org_id)
    ...
    return _tpl(request).TemplateResponse(
        "twitter/autoresponder/index.html",
        {
            "request": request,
            "user": request.session.get("user"),
            "page_id": "twitter_autoresponder",
            "personas": personas,
            "sessions": sessions,
            "accounts": accounts,
            "leads": leads,
...
async def personas_create(
    ...
    offer_summary: str = Form(default=""),
    tone: str = Form(default=""),
    knowledge_base: str = Form(default=""),
    cta_call_link: str = Form(default=""),
    cta_info_link: str = Form(default=""),
    forbidden_topics: str = Form(default=""),
):
    db = SessionLocal()
    try:
        personas_svc.create_persona(
            db, _org_id(request),
        ...
        db.close()
    return RedirectResponse("/twitter/autoresponder?notice=persona_created", status_code=303)
...
async def personas_archive(persona_id: int, request: Request):
    db = SessionLocal()
    try:
        personas_svc.archive_persona(db, persona_id)
    finally:
        db.close()
    return RedirectResponse("/twitter/autoresponder?notice=persona_archived", status_code=303)
...
async def sessions_create(
    ...
    lead_id: int = Form(...),
    persona_id: int = Form(...),
    goal: str = Form(default="qualify_lead"),
    approval_mode: str = Form(default="manual"),
):
    db = SessionLocal()
    try:
        # Find or create conversation by lead handle.
        from backend.modules.twitter.models import TwitterLead, TwitterConversation

        lead = db.get(TwitterLead, lead_id)
        if lead is None:
            return RedirectResponse("/twitter/autoresponder?notice=lead_not_found", status_code=303)
        conv = (
            db.query(TwitterConversation)
            .filter(
                TwitterConversation.account_id == account_id,
                TwitterConversation.organization_id == _org_id(request),
                TwitterConversation.counterpart_handle == lead.handle,
            )
            .first()
        )
        if conv is None:
            conv = TwitterConversation(
                ...
                lead_id=lead.id,
                counterpart_handle=lead.handle,
                counterpart_full_name=lead.full_name,
            )
            db.add(conv)
            db.flush()
        ...
        db.close()
    return RedirectResponse("/twitter/autoresponder?notice=session_created", status_code=303)
...
async def sessions_set_state(session_id: int, request: Request, state: str = Form(...)):
    ...
    try:
        ...
        db.close()
    return RedirectResponse(f"/twitter/autoresponder?notice=session_{state}", status_code=303)
...
async def sessions_set_approval(session_id: int, request: Request, approval_mode: str = Form(...)):
    ...
    try:
        ...
        db.close()
    return RedirectResponse(f"/twitter/autoresponder?notice=approval_{approval_mode}", status_code=303)


@router.post("/sessions/{session_id}/think-now")
async def sessions_think_now(session_id: int, request: Request):
    """Прогнать `think_next_response` сразу — чтобы оператор увидел draft."""
    db = SessionLocal()
    try:
        try:
        ...
        db.close()
    return RedirectResponse(f"/twitter/autoresponder?notice={qs}", status_code=303)
...
async def draft_approve_and_send(draft_id: int, request: Request):
    """Одобрить draft и сразу отправить через dm_sender."""
    from backend.modules.twitter.runtime.autoresponder_worker import _send_draft

    db = SessionLocal()
    try:
        draft = db.get(TwitterAIDialogDraft, int(draft_id))
        if draft is None:
            return RedirectResponse("/twitter/autoresponder?notice=draft_not_found", status_code=303)
        sess = ar_svc.get_session(db, draft.session_id)
        if sess is None:
            return RedirectResponse("/twitter/autoresponder?notice=session_missing", status_code=303)
...
async def draft_reject(draft_id: int, request: Request):
    ...
    try:
        ...
        db.close()
    return RedirectResponse("/twitter/autoresponder?notice=draft_rejected", status_code=303)
