"""Twitter runtime — общие helpers для всех sender'ов (try_locator, caps, marks)."""
...
from backend.modules.twitter.models import TwitterAccount, TwitterAccountDayUsage
...
def bump_usage(db: Session, account: TwitterAccount, field: str) -> None:
    today = datetime.now(timezone.utc).date().isoformat()
    row = (
        db.query(TwitterAccountDayUsage)
        .filter(TwitterAccountDayUsage.account_id == account.id, TwitterAccountDayUsage.usage_date == today)
        .one_or_none()
    )
    if row is None:
        row = TwitterAccountDayUsage(account_id=account.id, usage_date=today)
        db.add(row)
        db.flush()
    setattr(row, field, (getattr(row, field, 0) or 0) + 1)


def check_cap(db: Session, account: TwitterAccount, field: str, soft_cap: int) -> bool:
    today = datetime.now(timezone.utc).date().isoformat()
    row = (
        db.query(TwitterAccountDayUsage)
        .filter(TwitterAccountDayUsage.account_id == account.id, TwitterAccountDayUsage.usage_date == today)
...
def mark_account_login_required(db: Session, acc: TwitterAccount, reason: str) -> None:
...
def mark_account_restricted(db: Session, acc: TwitterAccount, reason: str) -> None:
    now = datetime.now(timezone.utc)
    acc.session_ok = False
    acc.last_login_check_at = now
    acc.login_blocked_at = now
