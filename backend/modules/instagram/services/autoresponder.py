"""Instagram AI autoresponder — порт Twitter autoresponder."""
...
from backend.modules.instagram.models import (
    InstagramAIDialogDraft, InstagramAIDialogSession, InstagramAIPersona,
...
_GOAL_DESCRIPTIONS = {
    "book_call": "Подвести собеседника к согласию на короткий созвон. Отправь ссылку cta_call_link и спроси удобное время.",
    "share_info": "Подвести к запросу материалов. Отправь cta_info_link.",
    "qualify_lead": "Узнать роль, интересы, бизнес/компанию. После 3-4 вопросов пометь как qualified и предложи следующий шаг.",
    "nurture": "Поддерживать тёплый диалог, не давить. Цель — оставить позитивное впечатление.",
}


def _build_persona_block(persona: InstagramAIPersona | None) -> str:
    if persona is None:
        return "Ты — представитель компании, отвечаешь на DM в Instagram. Пиши коротко, эмоционально, без штампов."
    parts: list[str] = []
    ...
    if persona.languages:
        parts.append(f"Языки: {', '.join(persona.languages)}.")
    if persona.knowledge_base:
        parts.append(f"Что ты знаешь:\n{persona.knowledge_base}")
    if persona.cta_call_link:
        parts.append(f"Ссылка на созвон (когда уместно): {persona.cta_call_link}")
    if persona.cta_info_link:
        parts.append(f"Ссылка на материалы (когда уместно): {persona.cta_info_link}")
    if persona.forbidden_topics:
        parts.append(f"НЕ обсуждай: {persona.forbidden_topics}")
    parts.append(
        "Стиль Instagram: пиши коротко (1-3 предложения), как живой человек. Можно эмодзи "
        "к месту (1-2 на сообщение), без формальных приветствий после первого сообщения."
    )
    return "\n\n".join(parts)


def _build_history_block(messages: list[InstagramMessage], limit: int = 12) -> str:
    last = messages[-limit:]
    rows = []
    for m in last:
...
def _build_lead_block(lead: InstagramLead | None) -> str:
    if lead is None:
        return ""
...
_OUTPUT_INSTRUCTION = (
    "Ответь СТРОГО JSON-объектом БЕЗ markdown-блока:\n"
    "{\n"
    '  "body": "твой ответ собеседнику в 1-3 предложениях",\n'
    '  "status": "continue|goal_reached|refused|silent",\n'
    '  "rationale": "краткое объяснение для оператора (на русском)"\n'
    "}\n"
    "status='goal_reached' — если по сути достиг цели сессии.\n"
    "status='refused' — если собеседник явно отказался / попросил не писать.\n"
    "status='silent' — если собеседник давно молчит.\n"
    ...
    for p in order:
        ...
        try:
            text, _meta = await complete_llm(
                ...
                system=system, user=user, response_mime_json=(p == "gemini"),
            )
...
def create_session(
    ...
    sess = InstagramAIDialogSession(
        organization_id=organization_id, account_id=account_id, lead_id=lead_id,
        conversation_id=conversation_id, persona_id=persona_id,
        goal=goal, approval_mode=approval_mode,
        scratchpad={}, next_check_at=datetime.now(timezone.utc),
    )
    db.add(sess)
    db.commit()
    return sess
...
def list_sessions(db: Session, organization_id: int) -> list[InstagramAIDialogSession]:
    return (
        db.query(InstagramAIDialogSession)
        .filter(InstagramAIDialogSession.organization_id == organization_id)
        .order_by(InstagramAIDialogSession.created_at.desc())
        .limit(500)
        .all()
    )


def get_session(db: Session, session_id: int) -> InstagramAIDialogSession | None:
    return db.get(InstagramAIDialogSession, int(session_id))


def update_session_state(db: Session, session_id: int, *, state: str | None = None, approval_mode: str | None = None) -> InstagramAIDialogSession | None:
...
async def think_next_response(db: Session, session_id: int) -> dict[str, Any]:
    ...
    persona = db.get(InstagramAIPersona, sess.persona_id) if sess.persona_id else None
    ...
    if sess.conversation_id:
        ...
        if conv is not None:
            messages = (
                ...
                .filter(InstagramMessage.conversation_id == conv.id)
                .order_by(InstagramMessage.sent_at.asc(), InstagramMessage.id.asc())
                .all()
            )
    ...
    user = (
        ...
        f"ОПИСАНИЕ: {_GOAL_DESCRIPTIONS.get(sess.goal, '')}\n\n"
    ...
    draft = InstagramAIDialogDraft(
        session_id=sess.id, body=body, rationale=rationale or None,
    ...
    if status == "goal_reached":
        ...
        sess.final_outcome = (
            "qualified" if sess.goal == "qualify_lead"
            else "call_booked" if sess.goal == "book_call"
            else "info_shared" if sess.goal == "share_info"
            else "nurtured"
        )
    elif status == "refused":
        sess.state = "stopped"
...
__all__ = [
    ...
    "think_next_response", "update_session_state",
]
