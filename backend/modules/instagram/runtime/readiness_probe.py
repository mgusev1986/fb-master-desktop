...
def probe_account(db: Session, account_id: int, *, headless: bool = True) -> dict[str, Any]:
    ...
    if not (acc.cookies_json or []):
        ...
        # автологина (см. backend.modules.instagram.services.playwright_login).
    ...
    try:
        with InstagramBrowser(acc, headless=headless) as session:
            ...
            try:
                ...
                acc.last_login_check_at = now
                acc.login_blocked_at = now
                acc.login_blocked_reason = f"Не удалось открыть instagram.com: {str(e)[:200]}"
                acc.status = "needs_attention"
                db.commit()
            ...
            if page_has_any_hint(body_excerpt, LOCALIZED_RESTRICTED_HINTS) or page_has_any_hint(final_url, LOCALIZED_RESTRICTED_HINTS):
                ...
                acc.last_login_check_at = now
                acc.login_blocked_at = now
                acc.login_blocked_reason = "Instagram пометил аккаунт как restricted/disabled."
            ...
            if page_has_any_hint(final_url, LOCALIZED_LOGIN_HINTS):
                ...
                acc.last_login_check_at = now
                acc.login_blocked_at = now
                acc.login_blocked_reason = "Instagram просит залогиниться (sessionid истёк)."
                acc.status = "needs_attention"
                db.commit()
            ...
            acc.last_login_check_at = now
            acc.login_blocked_at = now
            acc.login_blocked_reason = "Открылось, но навигация не обнаружена. Возможно, нужен re-auth."
            acc.status = "needs_attention"
            db.commit()
            return {"ok": False, "reason_code": "unknown_state", "url": final_url}
    except Exception as e:  # noqa: BLE001
        logger.exception("instagram readiness_probe failed account_id=%s", account_id)
        try:
            acc.session_ok = False
            acc.last_login_check_at = now
            acc.login_blocked_at = now
            acc.login_blocked_reason = f"Сбой playwright: {str(e)[:200]}"
            acc.status = "needs_attention"
            db.commit()
        except Exception:  # noqa: BLE001
            db.rollback()
