"""Instagram Direct conversations / inbox."""
...
router = APIRouter(prefix="/instagram/conversations")
...
async def conversations_index(request: Request):
    ...
    try:
        ...
        items = (
            ...
            .filter(InstagramConversation.organization_id == org_id)
            .order_by(InstagramConversation.last_message_at.desc().nullslast(), InstagramConversation.id.desc())
            .limit(500)
            .all()
        )
    finally:
    ...
    return _tpl(request).TemplateResponse(
        "instagram/conversations/list.html",
        {
            "request": request, "user": request.session.get("user"),
            "page_id": "ig_conversations",
            "conversations": items,
...
async def conversations_detail(conv_id: int, request: Request):
    ...
    try:
        org_id = _org_id(request)
        conv = db.get(InstagramConversation, int(conv_id))
        ...
        n = (
            ...
            .filter(
                ...
                InstagramMessage.read_at.is_(None),
            ).count()
        )
        if n:
            db.query(InstagramMessage).filter(
                InstagramMessage.conversation_id == conv.id,
                InstagramMessage.direction == "in",
                InstagramMessage.read_at.is_(None),
            ...
            conv.unread_count = 0
            db.commit()
        leads = db.query(InstagramLead).filter(InstagramLead.organization_id == org_id).limit(500).all()
        linked_lead = next((l for l in leads if l.id == conv.lead_id), None) if conv.lead_id else None
    finally:
        db.close()
    return _tpl(request).TemplateResponse(
        "instagram/conversations/detail.html",
        {
            "request": request, "user": request.session.get("user"),
            "page_id": "ig_conversations",
            "conversation": conv, "messages": msgs, "leads": leads,
            "linked_lead": linked_lead, "marked_read": n,
            "notice": request.query_params.get("notice"),
        },
    )
...
async def conversations_send(conv_id: int, request: Request, body: str = Form(...), headless: str = Form(default="on")):
    ...
    try:
        conv = db.get(InstagramConversation, int(conv_id))
        if conv is None:
            return RedirectResponse("/instagram/conversations?notice=conv_not_found", status_code=303)
        body = (body or "").strip()
        if not body:
            return RedirectResponse(f"/instagram/conversations/{conv_id}?notice=empty_body", status_code=303)
        target_handle = ""
        if conv.lead_id:
            lead = db.get(InstagramLead, conv.lead_id)
            target_handle = (lead.handle if lead else "") or ""
        if not target_handle:
            target_handle = (conv.counterpart_handle or "")
        if not target_handle:
            return RedirectResponse(f"/instagram/conversations/{conv_id}?notice=no_target", status_code=303)
        from backend.modules.instagram.runtime.dm_sender import send_dm

        try:
            res = send_dm(db, account_id=conv.account_id, to_handle=target_handle, body=body,
        ...
        db.close()
    return RedirectResponse(f"/instagram/conversations/{conv_id}?notice={qs}", status_code=303)
...
async def conversations_link_lead(conv_id: int, request: Request, lead_id: str = Form(default="")):
    ...
    try:
        ...
        if not lead_id or not lead_id.isdigit():
            ...
            lead = db.get(InstagramLead, int(lead_id))
            if lead is None or lead.organization_id != conv.organization_id:
                return RedirectResponse(f"/instagram/conversations/{conv_id}?notice=lead_not_found", status_code=303)
            conv.lead_id = lead.id
        db.commit()
    finally:
        db.close()
    return RedirectResponse(f"/instagram/conversations/{conv_id}?notice=linked", status_code=303)
