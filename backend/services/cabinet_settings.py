...
# Встроенный логин в Facebook через Electron webview (beta). Когда включён,
# кнопка «Войти в Facebook» открывает логин в webview внутри приложения, а не
# запускает отдельное окно Playwright Chromium. E2EE PIN для Messenger при этом
# сохраняется в том же Chromium-контексте, что и сам Messenger — второго запроса
# PIN при заходе в Мессенджер больше не будет. Старый Playwright-логин остаётся
# как fallback.
SK_EMBEDDED_LOGIN_ENABLED = "cabinet_embedded_login_enabled"
...
def effective_embedded_login_enabled(db: Session | None) -> bool:
    """По умолчанию False: используем старое Playwright окно входа.
    Включается флажком в «Настройках» — тогда «Войти» открывает Electron webview,
    что позволяет Messenger'у не просить повторно PIN E2EE."""
    if db is None:
        return False
    return _get_setting_value(db, SK_EMBEDDED_LOGIN_ENABLED) is True
...
def build_settings_page_context(request: Request, db: Session) -> dict[str, Any]:
    ...
    return {
        ...
        "cabinet_embedded_login_enabled": effective_embedded_login_enabled(db),
...
def save_settings_from_form(
    ...
    *,
    ...
    if form.get("embedded_login_enabled") == "1":
        _upsert_setting(db, SK_EMBEDDED_LOGIN_ENABLED, True)
    else:
        _delete_setting(db, SK_EMBEDDED_LOGIN_ENABLED)
